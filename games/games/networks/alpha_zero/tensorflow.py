...
class Losses(collections.namedtuple("Losses", "policy value l2")):
    ...
    def __str__(self):
        return ("Losses(total: {:.3f}, policy: {:.3f}, value: {:.3f}, " "l2: {:.3f})").format(
            self.total, self.policy, self.value, self.l2
        )

    def __add__(self, other):
        return Losses(self.policy + other.policy, self.value + other.value, self.l2 + other.l2)
...
class Model(object):
    ...
    @classmethod
    def build_model(cls, model_type, input_shape, output_size, nn_width, nn_depth, weight_decay, learning_rate, path):
        """Build a model with the specified params."""
        if model_type not in cls.valid_model_types:
            raise ValueError(f"Invalid model type: {model_type}, " f"expected one of: {cls.valid_model_types}")
        ...
        with g.as_default():
            cls._define_graph(model_type, input_shape, output_size, nn_width, nn_depth, weight_decay, learning_rate)
    ...
    def _define_graph(model_type, input_shape, output_size, nn_width, nn_depth, weight_decay, learning_rate):
        ...
        policy_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=policy_logits, labels=policy_targets), name="policy_loss"
        )
        ...
        l2_reg_loss = tf.add_n(
            [weight_decay * tf.nn.l2_loss(var) for var in tf.trainable_variables() if "/bias:" not in var.name],
            name="l2_reg_loss",
        )
    ...
    def write_graph(self, filename):
        ...
        tf.train.export_meta_graph(
            graph_def=self._session.graph_def, saver_def=self._saver.saver_def, filename=full_path, as_text=False
        )
        return full_path
    ...
    def inference(self, observation, legals_mask):
        return self._session.run(
            ...
            feed_dict={
                self._input: np.array(observation, dtype=np.float32),
    ...
    def get_value(self, x, legals_mask):
        value, _ = self.inference(x, legals_mask)
        return value

    def get_prior(self, x, legals_mask):
        _, prior = self.inference(x, legals_mask)
        return prior
    ...
    def save_checkpoint(self, step):
        return self._saver.save(self._session, os.path.join(self._path, "checkpoint"), global_step=step)
