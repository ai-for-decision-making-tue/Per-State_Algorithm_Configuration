...
class TrajectoryState(object):
    ...
    observation: list
    current_player: int
    legals_mask: list[int]
    action: int
    policy: list
    value: int
    scenario_bools: list[float] = field(init=False)

    def compute_scenario(self, scenario_list: list[Scenario], is_winning_move: bool) -> None:
        self.scenario_bools = [scenario.compute(self, is_winning_move) for scenario in scenario_list]


class Scenario(metaclass=ABCMeta):
    @abstractmethod
    def compute(self, trajectory_state: TrajectoryState, is_winning_move: bool) -> bool: ...


@dataclass
class WinningRowScenario(Scenario):
    row: int

    def compute(self, trajectory_state: TrajectoryState, is_winning_move: bool) -> bool:
        return is_winning_move and trajectory_state.action == self.row


nr_squares = 42


def rescale_observation(observation: list[float]) -> list[list[float], list[float], list[float]]:
    return [observation[i * nr_squares : (i + 1) * nr_squares] for i in range(3)]
...
class Trajectory(object):
    ...
    def __init__(self):
        self.states: list[TrajectoryState] = []
        self.returns = None
        self.scenario_list = [WinningRowScenario(i) for i in range(7)]

    def add(self, information_state, action, policy):
        self.states.append((information_state, action, policy))

    def compute_scenarios(self) -> None:
        for state in self.states[:-1]:
            state.compute_scenario(self.scenario_list, is_winning_move=False)

        state = self.states[-1]
        is_winning_move = self.returns[0] != 0.0
        state.compute_scenario(self.scenario_list, is_winning_move=is_winning_move)
...
def _play_game(logger, game_num, game, bots, temperature, temperature_drop):
    ...
    logger.print(
        "Game {}: Returns: {}; Actions: {}".format(game_num, " ".join(map(str, trajectory.returns)), " ".join(actions))
    )

    trajectory.compute_scenarios()
    return trajectory
    ...
    for game_num in itertools.count():
        ...
        logger.print(
            "AZ: {}, MCTS: {}, AZ avg/{}: {:.3f}".format(
                trajectory.returns[az_player], trajectory.returns[1 - az_player], len(results), np.mean(results.data)
            )
        )
    ...
    def collect_trajectories():
        ...
        for trajectory in trajectory_generator():
            ...
            replay_buffer.extend(
                model_lib.TrainInput(s.observation, s.legals_mask, s.policy, p1_outcome) for s in trajectory.states
            )
...
def alpha_zero(config: Config):
    ...
    evaluators = [
        spawn.Process(evaluator, kwargs={"game": game, "config": config, "num": i}) for i in range(config.evaluators)
    ]
