...
from warehousing.planning.helpers import stable_normalizer
...
class State():
    ...
    def evaluate(self, rollout_env, search_traj, policy, max_len):
        ...
        rollout_env.run_until_period_count(policy, search_traj, max_len)

        self.V = - search_traj.cumulative_return
...
class MCTS():
    ...
    def __init__(self, root, root_obs, gamma, search_env, search_traj, rollout_env=None, rollout_traj=None):
        ...
        self.rollout_traj = rollout_traj

    def set_random_seed(self, seed):
        random.seed(seed)

    # n_mcts: number of expanded leaves
    # n_chance_nodes: number of chance nodes per action
    # c: exploration parameter
    # policy: policy to be used in the rollout
    # start_state: initial state of the environment
    # start_return: initial return of the environment
    # max_len: maximum length of the rollout (in periods/steps)
    # max_time: maximum time of the search (in nanoseconds)
    def search(self, n_chance_nodes, c, policy, start_state, start_return, n_mcts=1000, max_len=10000,
               max_time=1000000000):
        ...
        while n_expanded_nodes < n_mcts:
            ...
            while True:
                ...
                # done = (start_period + self.search_traj.period_count >= max_len)
                ...
                if not done:
                    ...
                    # Evolve the trajectory based on the selected action
